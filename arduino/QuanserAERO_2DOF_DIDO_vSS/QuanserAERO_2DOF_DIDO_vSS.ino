...
const long sampleTime = 9000;  // set the sample time (the interval between SPI transactions) to 2000us = 2ms
...
int LEDGreen = 0;
int LEDBlue = 0;
// ============= Global variables DIDO controller ==============
// Control gain K used here was taken from simulink demo model and tested in a rev C Aero on Oct. 28. 2016
// Yeah what he said.

// The sizes that are used by all the arrays/matrices to be used later on in the for loops
#define ArraySize1 6
#define ArraySize2 2

// These arrays are pretty seft explanitory 
float Desired[ArraySize2] = {-0.5235, -0.7853};  // {pitch, yaw}
float CurState[ArraySize2] = {0, 0};
float Error[ArraySize1] = {0, 0, 0, 0, 0, 0};
float HiddenStates[ArraySize1] = {0, 0, 0, 0, 0, 0};
float MotorVol[ArraySize2]={0,0};

// Adding the controler ss as 2D-array to make calculations later on easier cuz imma lazy boi
// As such the matrices B and C are padded out with zeros to make all of them the same size.
// A matrix is 6-6
float K_A[ArraySize1][ArraySize1] = {
  {0.9997, 0, 0, 0, 0, 0},
  {0, 0.8187, -7.108e-8, -2.312e-8, 0.173, 0},
  {0, 0, 0.8187, 1.423e-8, 4.937e-4, 0},
  {0, 0, -1.423e-8, 0.8187, -1.518e-3, 0},
  {0, 0, 0, 0, 0.8187, 0},
  {0, 0, 0, 0, 0, 1}
};

// B matris is 6-2
float K_B[ArraySize1][ArraySize1] = {
  {-1.207, 1.9011e-11, 0, 0, 0, 0},
  {38.06, 34.76, 0, 0, 0, 0},
  {2.164, -61.19, 0, 0, 0, 0},
  {-1.624e7, 7.877e8, 0, 0, 0, 0},
  {-45.96, 61.98, 0, 0, 0, 0},
  {-2.665, 2.852e-11, 0, 0, 0, 0}
};

// C matrix is 2-6
float K_C[ArraySize1][ArraySize1] = {
  {-3.304e-13, 8.554, 17.53, 8.562e-8, -0.6693, -0.01347},
  {-3.979e-13, 11.18, 1.245, 2e-7, -1.247, -0.01653},
};

// D matrix is 2-2
// The D matrix is just zero so don't waste memory writing it down and instead waste time writing this comment

// Place holders for the timer variables used for the reference signal generator
unsigned long CurTime = 0;
unsigned long PrevTimePitch = 0;
unsigned long PrevTimeYaw = 0;
bool PitchOn = false;
bool YawOn = false;


// Variables used by the Display object so kept here just for that.
float Pitch_deg=0;
float Yaw_deg=0;
//============================================================
...
void loop() {
  ...
  if (currentMicros - previousMicros >= sampleTime) {
    ...
    //==============Setpoint Clock ===============
    ReferenceSignalGenerator();  // Creates different setpoits for Pitch and Yaw angles
    //============================================
...
//    }
  ...
  //  float encoder1Deg = (float)encoder1 * (360.0 / 2048.0);  // motor 1 angle
    ...
    //***********************************Start of Custom Code  for DIDO 2DOF controller **********************************************************
    // Made by Keitaro Nishimura so this is going to be buggy and suck so much hahahaahahahahahaha halp
           
    // Grab the current pitcha and yaw encoder values and convert from deg to rads for calculations later on
    CurState[0] = 0.01743*encoder2Deg; // Pitch
    CurState[1] = 0.01743*encoder3Deg; // Yaw

    // Calculate the error and send it to the function that calculates the Vmotors
    Error[0] = Desired[0] - CurState[0];
    Error[1] = Desired[1] - CurState[1];
    
    // Run the controller code to get the Vmotors values
    AeroDIDOcontroller();
    
    // Calculates Voltage for both motors
    motor0Voltage = MotorVol[0];
    motor1Voltage = MotorVol[1]; 
 //*****************************************************end*************************************************************************************
 //
 // set the saturation limit to +/- 24V for Motor0  as it is in Simulink model. 
 //
