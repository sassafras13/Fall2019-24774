...
data = load('UprightSysID.mat') ; 
time = data.ans(1,:) ; 
alphadata = data.ans(2,:) ; 
thetadata = data.ans(3,:) ; 
...
L_alpha = length(time) ; % Length of signal
L_theta = length(time) ;
...
Ualpha = fft(time);
Yalpha = fft(alphadata);

Utheta = fft(time) ; 
Ytheta = fft(thetadata) ; 
...
figure(2)
sgtitle('Sys ID Parametric Model for Upright Pendulum') ; 
