...
typedef RT_MODEL_ILC_2018_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * ILC_2018(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ILC_2018_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(ILC_2018_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://ILC_2018:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ILC_2018 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(ILC_2018_DW.HILReadEncoderTimebase_Task, (t_clock)
        ILC_2018_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
