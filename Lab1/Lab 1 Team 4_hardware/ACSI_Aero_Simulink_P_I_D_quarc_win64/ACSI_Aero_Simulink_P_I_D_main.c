...
typedef RT_MODEL_ACSI_Aero_Simulink_P_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * ACSI_Aero_Simulink_P_I_D(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ACSI_Aero_Simulink_P_I_D_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(ACSI_Aero_Simulink_P_I_D_DW.HILReadTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://ACSI_Aero_Simulink_P_I_D:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ACSI_Aero_Simulink_P_I_D -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(ACSI_Aero_Simulink_P_I_D_DW.HILReadTimebase_Task,
        (t_clock) ACSI_Aero_Simulink_P_I_D_P.HILReadTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
